...
- (void) modifyEventWithOptions:(CDVInvokedUrlCommand*)command {
    ...
    [self.commandDelegate runInBackground: ^{
        ...
        if (theEvent != nil) {
            ...
            NSString* timezone = [newCalOptions objectForKey:@"timezone"];
            if (timezone != (id) [NSNull null]) {
                theEvent.timeZone = [NSTimeZone timeZoneWithName:timezone];
            }
...
- (void)createEventWithOptions:(CDVInvokedUrlCommand*)command {
    ...
    [self.commandDelegate runInBackground: ^{
        ...
        NSString* timezone = [calOptions objectForKey:@"timezone"];
        if (timezone != (id) [NSNull null]) {
            myEvent.timeZone = [NSTimeZone timeZoneWithName:timezone];
        }
